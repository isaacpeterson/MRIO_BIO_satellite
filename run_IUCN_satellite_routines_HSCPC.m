function [domestic_satellite, global_satellite, satellite_params] = run_IUCN_satellite_routines_HSCPC(IUCN_data_object, IUCN_data_params, satellite_params)
    tic
    satellite_params = process_params(satellite_params, IUCN_data_object);
    
    disp('collapsing domestic tensors to user specification')
           
    [domestic_satellite, satellite_params] = build_domestic_satellite(IUCN_data_object, satellite_params, IUCN_data_params); 

    global_satellite = build_global_satellite(IUCN_data_object, satellite_params, IUCN_data_params);  
...
function [domestic_satellite, satellite_params] = build_domestic_satellite(IUCN_data_object, satellite_params, IUCN_data_params)
    
    disp('building domestic satellite...')

    if (satellite_params.write_aggregated_satellite_files == true) && (~exist([satellite_params.satellite_filepath], 'dir'))
        mkdir(satellite_params.satellite_filepath); 
    end

   domestic_satellite = aggregate_countries('domestic', satellite_params.domestic_threats_to_aggregate, satellite_params.build_domestic_satellite, IUCN_data_object, satellite_params, IUCN_data_params);
...
function global_satellite = build_global_satellite(IUCN_data_object, satellite_params, IUCN_data_params)
    
    disp('building global satellite...')
    global_threats = aggregate_countries('global', satellite_params.global_threats_to_aggregate, true, IUCN_data_object, satellite_params, IUCN_data_params);
...
function aggregated_satellite = aggregate_countries(satellite_type, threats_to_aggregate, build_satellite, IUCN_data_object, satellite_params, IUCN_data_params)
    ...
    if strcmp(IUCN_data_params.tensor_type, 'global')
        load([IUCN_data_params.tensor_folder, 'IUCN_tensor.mat'])   
        ...
        for country_index = country_indexes_to_use
            ...
            load([IUCN_data_params.tensor_folder, 'IUCN_tensor_', IUCN_data_object.IUCN_country_code_names{country_index}, '.mat'])
            ...
            if strcmp(satellite_type, 'domestic')
                ...
                if (satellite_params.write_aggregated_satellite_files == true) 
...
function collapsed_tensor = collapse_tensor(satellite_params, current_IUCN_tensor, threats_to_aggregate)
        ...
        if numel(satellite_params.q_row_to_use) > 1
            collapsed_tensor = collapsed_tensor(:, satellite_params.q_row_to_use, :); % select out species set
        else 
...
function [sorted_species_classification, sorted_threat_taxons] = select_and_sort_species(IUCN_data_object, satellite_params)
    ...
    sorted_threat_taxons = IUCN_threat_species_taxons(sorted_threat_species_indexes);
