...
%         consumption_country_set{country_index} = analyse_finalsale_output_routines(analyse_MRIO_params, IUCN_data_object, MRIO_threat_tensor, species_characteristics);
%     end
%    
...
% global_consumption = struct();
% global_consumption.industry_level.country_index_list = cellfun(@(x, y) repmat({y}, [size(x.industry_level.aggregated_paths, 1) 1]), consumption_country_set, num2cell((1:length(unique_countries))'), 'UniformOutput', false);
% global_consumption.industry_level.country_index_list = cell2mat(vertcat(global_consumption.industry_level.country_index_list{:}));
% 
% global_consumption.industry_level.country_list = cellfun(@(x, y) repmat({y}, [size(x.industry_level.aggregated_paths, 1) 1]), consumption_country_set, unique_countries, 'UniformOutput', false);
% global_consumption.industry_level.country_list = vertcat(global_consumption.industry_level.country_list{:});
% 
% global_consumption.industry_level.index_list = cellfun(@(x) num2cell(x.industry_level.aggregated_paths), consumption_country_set, 'UniformOutput', false);
% global_consumption.industry_level.index_list = cell2mat(vertcat(global_consumption.industry_level.index_list{:}));
% 
% global_consumption.industry_level.threat_intensities = cellfun(@(x) num2cell(x.industry_level.aggregated_path_vals), consumption_country_set, 'UniformOutput', false);
% global_consumption.industry_level.threat_intensities = cell2mat(vertcat(global_consumption.industry_level.threat_intensities{:}));
% 
% industry_link_indexes = arrayfun(@(x) find(global_consumption.industry_level.index_list == x), finalsale_data.industry_level.aggregated_paths, 'UniformOutput', false);
% 
% non_empties = cellfun('length', industry_link_indexes) > 0;
% industry_link_indexes = industry_link_indexes(non_empties);
% 
% grouped_industry_index_links = cellfun(@(x) [global_consumption.industry_level.country_list(x)...
%                                 IUCN_data_object.industry_characteristics.country_names_list(global_consumption.industry_level.index_list(x))  ...
%                                IUCN_data_object.industry_characteristics.commodity_classification_list(global_consumption.industry_level.index_list(x))], industry_link_indexes, 'UniformOutput', false);
% 
% 
% scaled_global_industry_threat_intensities = cellfun(@(x, y) [global_consumption.industry_level.threat_intensities(x) ./ (sum(global_consumption.industry_level.threat_intensities(x))) ...
%                                                             global_consumption.industry_level.threat_intensities(x) .* (y ./ sum(global_consumption.industry_level.threat_intensities(x)))], ...
...
% 
% writetable(consumption_finalsale_industry_level_table, [analyse_MRIO_params.output_folder 'global_consumption_finalsale_industry_level.txt'], 'delimiter', 'tab')
% 
...
% 
% 
% industry_link_indexes = arrayfun(@(x) find(global_consumption.industry_level.index_list == x), finalsale_data.industry_level.aggregates.aggregated_paths, 'UniformOutput', false);
% 
% non_empties = cellfun('length', industry_link_indexes) > 0;
...
%      
% grouped_industry_index_links = cellfun(@(x) [global_consumption.industry_level.country_index_list(x) repmat(global_consumption.industry_level.index_list(x), [1, 2])], industry_link_indexes_to_use, 'UniformOutput', false);
% 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% INTERNATIONAL INDUSTRY ASSESSMENT at global scale
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
% 
% finalsale_production = struct();
% finalsale_production.species_level.index_list = cellfun(@(x) num2cell(x.species_level.aggregated_paths), consumption_country_set, 'UniformOutput', false);
% finalsale_production.species_level.index_list = cell2mat(vertcat(finalsale_production.species_level.index_list{:}));
% 
% finalsale_production.species_level.threat_intensities = cellfun(@(x) num2cell(x.species_level.aggregated_path_vals), consumption_country_set, 'UniformOutput', false);
% finalsale_production.species_level.threat_intensities = cell2mat(vertcat(finalsale_production.species_level.threat_intensities{:}));
%                  
% 
% 
% %finalsale_production.species_level.species_list = cellfun(@(x) num2cell(x.species_level.aggregated_paths), consumption_country_set, 'UniformOutput', false);
% 
% consumption_finalsale_production_links = [global_consumption.species_level.country_list IUCN_data_object.industry_characteristics.country_index_list(finalsale_production.species_level.index_list)];
% 
% % international_inds = find( sum( abs(diff(consumption_finalsale_production_links(:, [1 3]), 1, 2)), 2) );
% international_inds = sum( abs(diff(consumption_finalsale_production_links(:, [1 3]), 1, 2)), 2) > 0;
% 
% net_international_trade_proportion = sum(finalsale_production.species_level.threat_intensities(international_inds))/sum(finalsale_production.species_level.threat_intensities);
% 
% global_consumption_blocks = cellfun(@(x) global_consumption.species_level.country_list == x, unique_country_inds, 'UniformOutput', false);
% international_consumption_blocks = cellfun(@(x) x & international_inds, global_consumption_blocks, 'UniformOutput', false);
% 
% 
% international_consumption_stats = struct();
% international_consumption_stats.country_level = cellfun(@(x, y) [sum(finalsale_production.species_level.threat_intensities(x)) sum(finalsale_production.species_level.threat_intensities(y))], ...
%     international_consumption_blocks, global_consumption_blocks, 'UniformOutput', false);
% 
% international_consumption_stats.country_level = vertcat(international_consumption_stats.country_level{:});
% 
% dom_consumption_trade_stats = international_consumption_stats.country_level(:, 2) - international_consumption_stats.country_level(:, 1);
% global_consumption_trade_stats = [dom_consumption_trade_stats international_consumption_stats.country_level international_consumption_stats.country_level(:, 1)./international_consumption_stats.country_level(:, 2)];
...
% 
% international_production_stats = cellfun(@(x, y) [sum(finalsale_production.species_level.threat_intensities(x)) sum(finalsale_production.species_level.threat_intensities(y))], ...
...
% 
% fid = fopen('~/Github/MRIO_BIO_SATELLITE/additional_population_data/developing_countries.txt');
...
% high_income_consumption_low_income_production_vals = cell2mat(cellfun(@(x) sum(finalsale_production.species_level.threat_intensities(x)), high_income_consumption_low_income_production_indexes, 'UniformOutput', false));
% 
...
% international_production_stats = cellfun(@(x, y) sum(global_finalsale_production.species_level.threat_intensities(x(y))), ...
...
% %%%%% CONSUMPTION to FINAL SALE INTERNATIONAL ROUTINES
% direct_dom_consumption_inds = strcmp(consumption_finalsale_industry_level(:, 2), consumption_finalsale_industry_level(:, 3));
% 
% consumption_finalsale_direct_international_industry_level = consumption_finalsale_industry_level((~direct_dom_consumption_inds), :);
% 
% international_consumption_finalsale_industry_level_table = cell2table(consumption_finalsale_direct_international_industry_level, 'VariableNames', ...
%     [{'global_rank', 'consumption_country', 'finalsale_country', 'finalsale_industry'}, analyse_MRIO_params.groups_to_count, {'aggregated_finalsale_threat_intensity', 'finalsale_threat_intensity_proportion', 'attributed_threat_intensity'}]);
% 
% writetable(international_consumption_finalsale_industry_level_table, [analyse_MRIO_params.output_folder 'consumption_finalsale_direct_international_industry_level.txt'], 'delimiter', 'tab')
% 
