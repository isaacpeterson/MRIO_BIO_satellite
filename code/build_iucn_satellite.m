function [satellite_object] = build_iucn_satellite(processed_iucn_data, satellite_params, system_type, industry_inputs)
    
    display(['building satellite to ' system_type ' specification'])
    
    satellite_object = struct();
    
    satellite_object.species_characteristics = build_species_characteristics(processed_iucn_data, satellite_params);
    
    satellite_object.satellite_characteristics = build_satellite_characteristics(satellite_params, processed_iucn_data, satellite_object.species_characteristics, ...
                                                                industry_inputs.industry_characteristics, industry_inputs.un_to_iucn_codes, processed_iucn_data.iucn_data.country_characteristics.iucn_country_code_names);
    
    if (satellite_params.build_direct_satellite == true)    
        [satellite_object.direct_satellite, satellite_object.satellite_characteristics] = build_direct_satellite(processed_iucn_data, satellite_params, satellite_object.species_characteristics, ...
                                                                                                                    satellite_object.satellite_characteristics, system_type);
    end
    
    if (satellite_params.build_greenhouse_satellite == true)
        satellite_object.greenhouse_satellite = build_greenhouse_satellite(processed_iucn_data, satellite_params, satellite_object.species_characteristics, ...
                                                                           satellite_object.satellite_characteristics, system_type);  
    end
    
    if ~strcmp(satellite_params.satellite_collapse_type, 'none')
        satellite_object.direct_satellite = run_collapse_satellite_routines('direct', satellite_params, satellite_object.satellite_characteristics, processed_iucn_data, species_characteristics, system_type);
        satellite_object.greenhouse_satellite = run_collapse_satellite_routines('greenhouse', satellite_params, satellite_object.satellite_characteristics, processed_iucn_data, species_characteristics, system_type);
...
function collapsed_satellite = run_collapse_satellite_routines(satellite_params, satellite_characteristics, processed_iucn_data, system_type, satellite_type)
    ...
    if strcmp(satellite_params.satellite_collapse_type, 'eora25')
        ...
        sector_classification_num = repmat(25, [processed_iucn_data.iucn_data.country_characteristics.ncoun, 1]);
    end
    
    if satellite_params.return_satellite == true
        collapsed_satellite = arrayfun(@(x) zeros(satellite_characteristics.classification_num, x), sector_classification_num, 'UniformOutput', false);  
    else collapsed_satellite = struct();
    end
    
    disp(['collapsing to ', satellite_params.satellite_collapse_type, '...'])
    
    for country_index = 1:processed_iucn_data.iucn_data.country_characteristics.ncoun
        
        if strcmp(satellite_params.output_file_type, 'mat')
        	current_country = load([satellite_params.satellite_filepath, system_type, '_', processed_iucn_data.iucn_data.country_characteristics.iucn_country_code_names{country_index}, ...
        ...
        elseif strcmp(satellite_params.output_file_type, 'csv') 
            csvread([satellite_params.satellite_filepath, system_type, '_', processed_iucn_data.iucn_data.country_characteristics.iucn_country_code_names{country_index}, ...
        ...
        disp([processed_iucn_data.iucn_data.country_characteristics.iucn_country_code_names{country_index}, ' collapsed at ' num2str(toc)])
...
function [current_satellite, satellite_characteristics] = sort_satellite(current_satellite, processed_iucn_data, satellite_params, satellite_characteristics)
                                                                                                                                                
    if strcmp(satellite_params.country_sort_type, 'none') 
        satellite_characteristics.sorted_country_names = processed_iucn_data.iucn_country_names;
    else
        if strcmp(satellite_params.country_sort_type, 'eora')
            disp('sorting to eora specification');
            [current_satellite, satellite_characteristics] = sort_satellite_to_eora(current_satellite, satellite_characteristics, processed_iucn_data.iucn_data.country_characteristics.iucn_country_names,  satellite_characteristics.classification_num);
        else
            [current_satellite, satellite_characteristics] = sort_satellite_by_country(current_satellite, processed_iucn_data, satellite_params, satellite_characteristics);
        end
    end
...
function satellite_characteristics = build_satellite_characteristics(satellite_params, processed_iucn_data, species_characteristics, industry_characteristics, un_to_iucn_codes, iucn_country_code_names)
    ...
    if strcmp(satellite_params.country_sort_type, 'eora')
    	satellite_characteristics.country_characteristics = find_eora_industry_specification(industry_characteristics, un_to_iucn_codes, iucn_country_code_names); 
    end       
...
function [expanded_satellite, satellite_characteristics] = sort_satellite_to_eora(current_satellite, satellite_characteristics, iucn_country_names, classification_num) 
    ...
    eora_mapping_vector = satellite_characteristics.country_characteristics.mapping_vector;
    
    dummy_inds = find(eora_mapping_vector == 0); 
    dummy_country_names = satellite_characteristics.country_characteristics.country_names(dummy_inds);    
    ...
    sector_matches = (sector_lengths' == satellite_characteristics.country_characteristics.industry_specification(:, 1));
        
    dummy_satellite = build_dummy_satellite(eora_mapping_vector, sector_matches, classification_num, satellite_characteristics.country_characteristics.industry_specification);
...
function [current_satellite, satellite_characteristics] = sort_satellite_by_country(current_satellite, processed_iucn_data, satellite_params, satellite_characteristics) 
    ...
    industry_threat_counts = cellfun(@(x) sum(x, 2), current_satellite, 'UniformOutput', false); 
        ...
        elseif strcmp(satellite_params.country_sort_type, 'GDP')
            metric_to_sort = cellfun(@(x) sum(x(:)), processed_iucn_data.x); 
        end
        
        [~, sorted_country_inds] = sort(metric_to_sort);
 
        current_satellite = current_satellite(sorted_country_inds);
        satellite_characteristics.sorted_country_names = processed_iucn_data.iucn_country_names(sorted_country_inds);
...
function [direct_satellite, satellite_characteristics] = build_direct_satellite(processed_iucn_data, satellite_params, species_characteristics, satellite_characteristics, ...
                                                                                  system_type)
   ...
   direct_satellite = aggregate_iucn_tensor('direct', satellite_characteristics.direct_threats, processed_iucn_data, satellite_params, species_characteristics, satellite_characteristics, system_type);
     
   [direct_satellite, satellite_characteristics] = sort_satellite(direct_satellite, processed_iucn_data, satellite_params, satellite_characteristics);
...
function greenhouse_satellite = build_greenhouse_satellite(processed_iucn_data, satellite_params, species_characteristics, satellite_characteristics, system_type)
    
    disp('building global satellite...')
    greenhouse_threats = aggregate_iucn_tensor('greenhouse', satellite_characteristics.greenhouse_threats, processed_iucn_data, satellite_params, species_characteristics, satellite_characteristics, system_type);
    greenhouse_threats = sum([greenhouse_threats{:}], 2);
    species_num = satellite_characteristics.classification_num;
    ghg_block = cellfun(@(x) (repmat(x', [species_num, 1])/processed_iucn_data.industry_data.global_ghg), processed_iucn_data.industry_data.ghg, 'UniformOutput', false);
    
    greenhouse_satellite = cellfun(@(x) x .* repmat(greenhouse_threats, [1, size(x, 2)]), ghg_block,'UniformOutput', false);
    
    [greenhouse_satellite, ~] = sort_satellite(greenhouse_satellite, processed_iucn_data, satellite_params, satellite_characteristics);
    
    disp(['global satellite built at ', num2str(toc)])
    
end

function current_satellite = aggregate_iucn_tensor(satellite_type, threats_to_aggregate, processed_iucn_data, satellite_params, species_characteristics, satellite_characteristics, system_type)
    
    if (satellite_params.return_satellite == true)
        
        if strcmp(satellite_type, 'direct')
            current_satellite = arrayfun(@(x) zeros(satellite_characteristics.classification_num, x), satellite_characteristics.sector_lengths, 'UniformOutput', false);  
        else current_satellite = arrayfun(@(x) zeros(satellite_characteristics.classification_num, 1), satellite_characteristics.sector_lengths, 'UniformOutput', false); 
        end
     ...
     if strcmp(satellite_params.tensor_scale, 'country')
        ...
        country_indexes_to_use = 1:processed_iucn_data.iucn_data.country_characteristics.ncoun;
        
        for country_index = country_indexes_to_use
            
            load([satellite_params.iucn_tensor_file_prefix, system_type, '_', processed_iucn_data.iucn_data.country_characteristics.iucn_country_code_names{country_index}, '.mat'])
            %load([satellite_params.iucn_tensor_file_prefix, processed_iucn_data.iucn_data.country_characteristics.iucn_country_code_names{country_index}, '.mat'])

            collapsed_tensor = collapse_species_tensor(satellite_params, satellite_characteristics, species_characteristics, current_iucn_tensor, threats_to_aggregate);
        
            if strcmp(satellite_type, 'direct')
                current_sector_length = satellite_characteristics.sector_lengths(country_index);
            
                current_aggregated_country = zeros(satellite_characteristics.classification_num, current_sector_length);
            ...
            if (satellite_params.write_satellite_to_disk == true) 
                ...
                if strcmp(satellite_params.output_file_type, 'mat')
                    save([satellite_params.output_satellite_dir, system_type, '_', processed_iucn_data.iucn_data.country_characteristics.iucn_country_code_names{country_index}, '_', satellite_type, '_satellite.mat'], 'current_aggregated_country')
                elseif strcmp(satellite_params.output_file_type, 'csv')
                    csvwrite([satellite_params.output_satellite_dir, system_type, '_', processed_iucn_data.iucn_data.country_characteristics.iucn_country_code_names{country_index}, '_',  satellite_type, '_satellite.csv'], current_aggregated_country)
                end
            ...
            disp([processed_iucn_data.iucn_data.country_characteristics.iucn_country_code_names{country_index} ' ' satellite_type ' threats satellite built at ' num2str(toc)])
        ...
        if strcmp(satellite_type, 'direct')
            for country_ind = 1:processed_iucn_data.iucn_data.country_characteristics.ncoun;
                current_satellite{country_ind}(:, :) = collapsed_tensor(country_ind, :, 1:satellite_characteristics.sector_lengths(country_ind));
            end
        else 
...
%     species_characteristics.iucn_threat_taxons = processed_iucn_data.iucn_data.iucn_threat_data.iucn_threat_taxons(sorted_species_indexes);  
%     species_characteristics.species_names = processed_iucn_data.iucn_data.iucn_threat_data.iucn_species_name(sorted_species_indexes); 
%     
