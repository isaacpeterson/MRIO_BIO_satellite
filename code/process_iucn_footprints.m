function consumption_footprint_characteristics = process_iucn_footprints(industry_inputs, footprint_input_objects, analyse_footprint_params, build_footprint_params)
...
%                                                                                  footprint_objects.satellite_collapse_groups{x}, analyse_footprint_params.species_group_weights(x)), ...
%                                                                                  num2cell(1:numel(footprint_objects.species_group_names))', 'un', false);
                            
    disp('processing footprints at consumption level...')
    
    consumption_footprint_characteristics = cellfun(@(x) build_consumption_level_footprints(analyse_footprint_params, build_footprint_params, industry_inputs, unique_countries, ...
                                                                            country_indexes_to_use, x), num2cell(1:numel(footprint_input_objects.species_group_names))', 'un', false);
...
function consumption_finalsale_matches = match_consumption_to_finalsale(consumption_footprint_data, processed_finalsale_data, country_indexes_to_use)
    ...
    [consumption_finalsale_matches.consumption_commodity, consumption_index, ~] = arrayfun(@(x) intersect(consumption_footprint_data.finalsale_data_grouped_by_consumption_country{x}.aggregated_sector_scale.aggregated_paths, ...
                                                                                                          processed_finalsale_data.aggregated_sector_scale.aggregated_paths), (1:numel(country_indexes_to_use))', 'un', false);
    
    consumption_finalsale_matches.consumption_threat_intensities = cellfun(@(x, y) consumption_footprint_data.finalsale_data_grouped_by_consumption_country{x}.aggregated_sector_scale.aggregated_path_vals(y), ...
...
function merged_finalsale_data = build_scaled_threat_intensities(consumption_country_grouped_by_commodity, finalsale_data, ...
    ...
    consumption_country_grouped_by_commodity_intensities = consumption_country_grouped_by_commodity.grouped_path_vals;
    merged_finalsale_data.scaled_threat_proportion = cellfun(@(x) x./(sum(x)), consumption_country_grouped_by_commodity_intensities, 'un', false);  
...
function consumption_footprint_data = build_consumption_level_footprints(analyse_footprint_params, build_footprint_params, industry_inputs, ...
                                                                                unique_countries, country_indexes_to_use, species_group_to_use)
    
    consumption_footprint_data = struct();
    
    consumption_footprint_data.finalsale_data_grouped_by_consumption_country = build_consumption_level_footprint_group('consumption', ...
                                                                                                                        analyse_footprint_params, ...
    ...
    species_present = find(cellfun('length', consumption_footprint_data.finalsale_data_grouped_by_consumption_country));  
    ...
    if ~isempty(species_present)
        ...
        consumption_footprint_data.finalsale_data_grouped_by_consumption_country = consumption_footprint_data.finalsale_data_grouped_by_consumption_country(species_present);   
        
        consumption_footprint_data.aggregated_sector_scale = expand_aggregated_sector_scale_data(consumption_footprint_data.finalsale_data_grouped_by_consumption_country, ...
                                                                                                 country_indexes_to_use, ... 
                                                                                                 unique_countries(species_present));
                                                                                             
        consumption_footprint_data.sector_to_sector_scale = expand_sector_to_sector_finasale_data(consumption_footprint_data.finalsale_data_grouped_by_consumption_country, ...
                                                                                                  country_indexes_to_use, ...
                                                                                                  industry_inputs.industry_characteristics);
...
function current_footprint = select_footprint_subset(current_footprint, analyse_footprint_params, species_group_to_use, industry_characteristics)
...
%     current_footprint = structfun(@(x) vertcat(x), current_footprint);
    ...
    if ( ~isempty(current_footprint.species) && ~strcmp(analyse_footprint_params.country_of_interest, 'global'))
...
function country_scale_consumption = build_consumption_level_footprint_group(assessment_scale, analyse_footprint_params, build_footprint_params, industry_characteristics, country_indexes_to_use, species_group_to_use)
    ...
    for country_index = 1:country_num
        ...
        current_footprint = load([build_footprint_params.footprint_filename_prefix assessment_scale '_' industry_characteristics.unique_country_codes{country_indexes_to_use(country_index)}  ...
...
function aggregated_sector_scale = expand_aggregated_sector_scale_data(consumption_country_set, country_indexes_to_use, included_countries)
...
function consumption_based_sector_to_sector_scale = expand_sector_to_sector_finasale_data(consumption_country_set, country_indexes_to_use, industry_characteristics)
    
    consumption_based_sector_to_sector_scale = struct();
    
    consumption_based_sector_to_sector_scale.finalsale_to_production_sectors = cellfun(@(x) num2cell(x.sector_to_sector_scale.aggregated_paths), consumption_country_set, 'un', false); 
    consumption_based_sector_to_sector_scale.finalsale_to_production_threat_intensities = cellfun(@(x) num2cell(x.sector_to_sector_scale.aggregated_path_vals), consumption_country_set, 'un', false); 
    consumption_based_sector_to_sector_scale.consumption_country_list = cellfun(@(x, y) repmat(y, [size(x, 1) 1]), consumption_based_sector_to_sector_scale.finalsale_to_production_sectors, num2cell(country_indexes_to_use), 'un', false);
   
    consumption_based_sector_to_sector_scale.finalsale_to_production_sectors = cell2mat(vertcat(consumption_based_sector_to_sector_scale.finalsale_to_production_sectors{:}));
    consumption_based_sector_to_sector_scale.finalsale_to_production_threat_intensities = cell2mat(vertcat(consumption_based_sector_to_sector_scale.finalsale_to_production_threat_intensities{:}));
    consumption_based_sector_to_sector_scale.consumption_country_list = vertcat(consumption_based_sector_to_sector_scale.consumption_country_list{:});
        
    consumption_based_sector_to_sector_scale.finalsale_country_list = industry_characteristics.country_index_list(consumption_based_sector_to_sector_scale.finalsale_to_production_sectors(:, 1));
    consumption_based_sector_to_sector_scale.production_country_list = industry_characteristics.country_index_list(consumption_based_sector_to_sector_scale.finalsale_to_production_sectors(:, 2));
    
    consumption_based_sector_to_sector_scale.international_indexes = sum( abs(diff([consumption_based_sector_to_sector_scale.consumption_country_list consumption_based_sector_to_sector_scale.finalsale_country_list consumption_based_sector_to_sector_scale.production_country_list], 1, 2)), 2) > 0;
...
function country_scale_net_trade_characteristics = build_country_scale_net_trade_table(trade_characteristics, included_countries)
    ...
    country_scale_net_trade_characteristics.data_blocknames = [trade_characteristics.country_scale.consumption_characteristics.data_blocknames(1:(end - 1)) ...
                                                          {'low_income_imports', 'exports', 'net'} trade_characteristics.country_scale.consumption_characteristics.data_blocknames(end)];
...
function country_scale_trade_characteristics = assess_international_trade_characteristics(assess_type, consumption_footprint_data, included_countries, country_indexes_to_use)
    ...
    country_scale_trade_characteristics.data_blocknames = {'domestic', 'international', 'total'};
    country_scale_trade_characteristics.table = cell2table([included_countries num2cell(country_scale_trade_characteristics.data_block) num2cell(country_scale_trade_characteristics.partitioned_threat_intensities.international_trade_proportion)],...
                                                            'VariableNames', [{'Country'}, country_scale_trade_characteristics.data_blocknames  {'international_proportion'}]);
                                                        
    
%     country_scale_trade_characteristics.data_block = [species_counts ...
...
%     
%     country_scale_trade_characteristics.data_blocknames = [species_group_names', {'domestic', 'international', 'total'}];
%     country_scale_trade_characteristics.table = cell2table([included_countries num2cell(country_scale_trade_characteristics.data_block) num2cell(country_scale_trade_characteristics.international_trade_proportion)],...
%                                                            'VariableNames', [{'Country'}, country_scale_trade_characteristics.data_blocknames  {'international_proportion'}]);
...
% function country_scale_trade_characteristics = assess_international_trade_characteristics(assess_type, consumption_footprint_data, merged_data, included_countries, country_indexes_to_use, species_counts, species_group_names)
%     
%     if strcmp(assess_type, 'consumption')
%         country_index_list = consumption_footprint_data.sector_to_sector_scale.consumption_country_list;
%     else
...
% 
% 
%     global_blocks = cellfun(@(x) country_index_list == x, num2cell(country_indexes_to_use), 'un', false);
%     
%     international_blocks = cellfun(@(x) x & consumption_footprint_data.sector_to_sector_scale.international_indexes, global_blocks, 'un', false);
%    
%     threat_intensities_international = cellfun(@(x) sum(consumption_footprint_data.sector_to_sector_scale.finalsale_to_production_threat_intensities(x)), international_blocks, 'un', false);
%     
%     country_scale_trade_characteristics = struct();                                                                    
%     country_scale_trade_characteristics.threat_intensities_international = vertcat(threat_intensities_international{:});
%     
%     country_scale_trade_characteristics.threat_intensities_global = cellfun(@(x) sum(consumption_footprint_data.sector_to_sector_scale.finalsale_to_production_threat_intensities(x)), global_blocks, 'un', false);
...
function import_characteristics = assess_imports_by_economy_level(income_type, trade_characteristics, international_trade_indexes, low_income_internal_indexes, high_income_internal_indexes, country_indexes_to_use, unique_countries)
    ...
    import_characteristics.imported_proportion = import_characteristics.imported_total ./ trade_characteristics.country_scale.consumption_characteristics.partitioned_threat_intensities.international;
