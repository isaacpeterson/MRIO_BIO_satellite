function consumption_footprint_characteristics = process_iucn_footprints(country_of_interest, impact_assessment_level, industry_inputs, footprint_input_objects, ...
                                                                        analyse_footprint_params, build_footprint_params)
...
function consumption_footprint_data = build_consumption_level_footprints(country_of_interest, impact_assessment_level, analyse_footprint_params, build_footprint_params, industry_inputs, ...
    ...
    if ~isempty(species_present)
        ...
        consumption_footprint_data.country_scale.consumption = assess_trade_characteristics('consumption', ...
                                                                                                            consumption_footprint_data,...
                                                                                                            unique_countries(species_present), ...
                                                                                                            country_indexes_to_use); 
                                                                                                                      
        consumption_footprint_data.country_scale.production = assess_trade_characteristics('production', ...
                                                                                                               consumption_footprint_data, ...
                                                                                                               unique_countries(species_present), ...
                                                                                                               country_indexes_to_use);
                                                                                       
        consumption_footprint_data.country_scale.imports = build_import_trade_characteristics(consumption_footprint_data, ...
                                                                                                             industry_inputs.low_income_country_names, ...   
                                                                                                             unique_countries(species_present), ....
                                                                                                             country_indexes_to_use);
    
        consumption_footprint_data.country_scale = build_net_country_scale(consumption_footprint_data.country_scale, country_indexes_to_use);     
...
function global_scale_characteristics = build_global_scale_characteristics(trade_characteristics)
    ...
    global_scale_characteristics.trade_stats.total_global_trade = sum(trade_characteristics.sector_to_sector_scale.threat_intensities);
    global_scale_characteristics.trade_stats.total_international_trade = sum(trade_characteristics.sector_to_sector_scale.threat_intensities(trade_characteristics.sector_to_sector_scale.international_indexes));
    ...
    global_scale_characteristics.international_trade.low_income = calc_global_stats(trade_characteristics.country_scale.imports.low_income, ...
                                                                                    global_scale_characteristics.trade_stats.total_international_trade);
     global_scale_characteristics.international_trade.high_income = calc_global_stats(trade_characteristics.country_scale.imports.high_income, ...
                                                                                    global_scale_characteristics.trade_stats.total_international_trade);
...
function aggregated_sector_scale = expand_aggregated_sector_scale_data(consumption_country_set, country_indexes_to_use, industry_characteristics, impact_assessment_level, country_of_interest)
    ...
    aggregated_sector_scale.consumption_country = cellfun(@(x, y) repmat(y, [size(x.aggregated_sector_scale.aggregated_paths, 1) 1]), consumption_country_set, num2cell(country_indexes_to_use), 'un', false);
    aggregated_sector_scale.paths = cellfun(@(x) x.aggregated_sector_scale.aggregated_paths, consumption_country_set, 'un', false);
    ...
    if strcmp(impact_assessment_level, 'consumption') && ~strcmp(country_of_interest, 'global')
        ...
        current_industry_set = ismember(aggregated_sector_scale.consumption_country, consumption_countries_to_use);
        aggregated_sector_scale = structfun(@(x) x(current_industry_set), aggregated_sector_scale, 'un', false);
    end
    
    datablock = [industry_characteristics.unique_countries(aggregated_sector_scale.consumption_country), ...
                 industry_characteristics.country_names_list(aggregated_sector_scale.paths), ...
                 industry_characteristics.commodity_classification_list(aggregated_sector_scale.paths), ...
                 num2cell(aggregated_sector_scale.threat_intensities)];
      
    [~, sorted_indexes] = sort(aggregated_sector_scale.threat_intensities, 'descend');
    aggregated_sector_scale.table = cell2table(datablock(sorted_indexes, :), 'VariableNames', {'consumption_country', 'finalsale_country', 'finalsale_industry', 'threat_intensity'});
...
function sector_to_sector_scale = expand_sector_to_sector_data(consumption_country_set, country_indexes_to_use, industry_characteristics, impact_assessment_level, country_of_interest)
    ...
    sector_to_sector_scale.paths = cellfun(@(x) x.sector_to_sector_scale.aggregated_paths, consumption_country_set, 'un', false); 
    sector_to_sector_scale.threat_intensities = cellfun(@(x) x.sector_to_sector_scale.aggregated_path_vals, consumption_country_set, 'un', false);
    sector_to_sector_scale.consumption_country = cellfun(@(x, y) repmat(y, [size(x, 1) 1]), sector_to_sector_scale.paths, num2cell(country_indexes_to_use), 'un', false);

    sector_to_sector_scale = structfun(@(x) vertcat(x{:}), sector_to_sector_scale, 'un', false);
        
    finalsale_country_list = industry_characteristics.country_index_list(sector_to_sector_scale.paths(:, 1));
    sector_to_sector_scale.production_country_list = industry_characteristics.country_index_list(sector_to_sector_scale.paths(:, 2));
    
     if strcmp(impact_assessment_level, 'consumption') && ~strcmp(country_of_interest, 'global')
        consumption_countries_to_use = find(ismember(industry_characteristics.unique_countries, country_of_interest));
        current_industry_set = ismember(sector_to_sector_scale.consumption_country, consumption_countries_to_use);
        sector_to_sector_scale = structfun(@(x) x(current_industry_set), sector_to_sector_scale, 'un', false);
     end
    
    sector_to_sector_scale.international_indexes = sum( abs(diff([sector_to_sector_scale.consumption_country ...
                                                                  finalsale_country_list ...
                                                                  sector_to_sector_scale.production_country_list], 1, 2)), 2) > 0;
     
   sector_to_sector_scale.datablock = [industry_characteristics.country_names_list(sector_to_sector_scale.consumption_country), ...
                                         industry_characteristics.country_names_list(sector_to_sector_scale.paths(:, 1)), ...
                                         industry_characteristics.commodity_classification_list(sector_to_sector_scale.paths(:, 1)), ...
                                         industry_characteristics.country_names_list(sector_to_sector_scale.paths(:, 2)), ...
                                         industry_characteristics.commodity_classification_list(sector_to_sector_scale.paths(:, 2)), ...
                                         num2cell(sector_to_sector_scale.threat_intensities)];
    
    [~, sorted_indexes] = sort(sector_to_sector_scale.threat_intensities, 'descend');
    sector_to_sector_scale.datablock_names = {'consumption_country', 'finalsale_country', 'finalsale_industry', ...
                                              'production_country', 'production_industry', 'threat_intensity'};
                                                                        
    sector_to_sector_scale.table = cell2table(sector_to_sector_scale.datablock(sorted_indexes, :), 'VariableNames', sector_to_sector_scale.datablock_names);
                                                             
end


function country_scale = build_net_country_scale(country_scale, countries_to_use)
 
    country_scale.consumption_country = countries_to_use;
    country_scale.import_export_discriminator = country_scale.consumption.partitioned_threat_intensities.international ...
                                                                            - country_scale.production.partitioned_threat_intensities.international;   
                                            
    country_scale.threat_intensities = country_scale.consumption.partitioned_threat_intensities.global ...
                                       - country_scale.production.partitioned_threat_intensities.international;
                                       
%     footprint_data.country_scale.datablock = [footprint_data.country_scale.consumption.datablock(:, 1:(end - 1))...
%                                                      footprint_data.country_scale.import_characteristics.low_income.imported_total...
%                                                      footprint_data.country_scale.production.partitioned_threat_intensities.international...
%                                                      footprint_data.country_scale.consumption.datablock(:, end)...
%                                                      country_scale.net_threat_intensity];
%                                                  
%     footprint_data.country_scale.datablock_names = [footprint_data.country_scale.consumption.datablock_names(1:(end - 1)) ...
%                                                           {'low_income_imports', 'exports', 'gross'} footprint_data.country_scale.consumption.datablock_names(end)];
% 
%     footprint_data.country_scale.table = cell2table( [included_countries num2cell([country_scale.datablock ...
%                                                      footprint_data.country_scale.import_characteristics.low_income.imported_proportion...
%                                                      footprint_data.country_scale.production.partitioned_threat_intensities.international_trade_proportion...
%                                                      footprint_data.country_scale.consumption.partitioned_threat_intensities.international_trade_proportion])],...
%                                                      'VariableNames', [{'Country'} country_scale.datablock_names {'low_income_imported_proportion', 'proportion_exported', 'international_consumption_proportion'}] );
%     
end


function country_scale_trade_characteristics = assess_trade_characteristics(assess_type, consumption_footprint_data, included_countries, country_indexes_to_use)
    
    if strcmp(assess_type, 'consumption')
        country_index_list = consumption_footprint_data.sector_to_sector_scale.consumption_country;   
    elseif strcmp(assess_type, 'production')
    ...
    country_scale_trade_characteristics.trade_paths = included_countries;
    ...
    country_scale_trade_characteristics.partitioned_threat_intensities = structfun(@(x) cellfun(@(y) sum(consumption_footprint_data.sector_to_sector_scale.threat_intensities(y)), x), ...
                                                                                        country_scale_trade_characteristics.partitioned_indexes, 'un', false);
    
    country_scale_trade_characteristics.international_trade_proportion = country_scale_trade_characteristics.partitioned_threat_intensities.international ./ ...
                                                                         (country_scale_trade_characteristics.partitioned_threat_intensities.global + 1e-10);
        
    datablock = [country_scale_trade_characteristics.partitioned_threat_intensities.domestic ...
    ...
    country_scale_trade_characteristics.datablock_names = {'domestic', 'international', 'net'};
    
    country_scale_trade_characteristics.table = cell2table([included_countries num2cell(datablock) num2cell(country_scale_trade_characteristics.international_trade_proportion)],...
                                                            'VariableNames', [{'Country'}, country_scale_trade_characteristics.datablock_names  {'international_proportion'}]);
end
...
function import_characteristics = build_import_trade_characteristics(trade_characteristics, low_income_country_names, included_countries, country_indexes_to_use)
    ...
    import_characteristics.low_income = assess_imports_by_economy_level('low', ...
                                                                        ...
                                                                        trade_characteristics.country_scale.consumption.partitioned_indexes.international, ...
...
function import_characteristics = assess_imports_by_economy_level(income_type, trade_characteristics, international_trade_indexes, low_income_internal_indexes, high_income_internal_indexes, country_indexes_to_use, unique_countries)
    ...
    import_characteristics.imported_total = cellfun(@(x) sum(trade_characteristics.sector_to_sector_scale.threat_intensities(x)), import_characteristics.import_indexes);
    ...
    import_characteristics.imported_proportion = import_characteristics.imported_total ./ ...
                                                 (trade_characteristics.country_scale.consumption.partitioned_threat_intensities.international + 1e-10);
    
    import_characteristics.datablock = [import_characteristics.imported_total trade_characteristics.country_scale.consumption.partitioned_threat_intensities.global];
    import_characteristics.datablock_names = {'imports', 'net_consumption'}; 
    import_characteristics.table = cell2table([unique_countries num2cell(import_characteristics.datablock) num2cell(import_characteristics.imported_proportion)], ...
                                               'VariableNames', [{'Country'}, import_characteristics.datablock_names {'proportional_imports'}]);
end
...
%                                                                                       consumption_footprint_data.aggregated_sector_scale.paths, ... 
%                                                                                       consumption_footprint_data.aggregated_sector_scale.consumption_country, ... 
...
%                                                                                     industry_inputs.industry_characteristics);
%
%             merged_data.country_scale.species_counts = build_country_scale_species_counts(consumption_footprint_data.aggregated_sector_scale.paths, ... 
%                                                                                       consumption_footprint_data.aggregated_sector_scale.consumption_country, ... 
%                                                                                       processed_finalsale_data, ...
%                                                                                       country_indexes_to_use, ...
%                                                                                       species_groups);
%
%         merged_data.country_scale.consumption = assess_international_trade_characteristics('consumption', ...
%                                                                                                             consumption_footprint_data,...
...
%     
%     finalsale_link_indexes = arrayfun(@(x) find(consumption_footprint_data.aggregated_sector_scale.paths == x), finalsale_data.aggregated_sector_scale.aggregated_paths, 'un', false);
% 
...
% 
%     expanded_finalsale_commodity_names = cellfun(@(x) industry_characteristics.commodity_classification_list(consumption_footprint_data.aggregated_sector_scale.paths(x)), ...
%                                                                         finalsale_link_indexes, 'un', false);
% 
%     expanded_finalsale_country_names = cellfun(@(x) industry_characteristics.country_names_list(consumption_footprint_data.aggregated_sector_scale.paths(x)), ...
%                                                                         finalsale_link_indexes, 'un', false);    
%                                                                     
...
%     end  
%     
%     merged_aggregated_sector_scale.datablock = [vertcat(consumption_country_names_per_finalsale_commodity{:})...
...
%                                                                         merged_aggregated_sector_scale.datablock(indexes_sorted_by_threat_intensity, :)], ...
