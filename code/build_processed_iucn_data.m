function processed_data = build_processed_iucn_data(iucn_data_params, un_to_iucn_codes, industry_characteristics, global_params)
    
    tic
    
    if (iucn_data_params.process_iucn_data == false) && exist([iucn_data_params.processed_iucn_data_filename_prefix, system_type, '.mat'], 'file')
        disp(['loading processed iucn data from ', [iucn_data_params.processed_iucn_data_filename_prefix, system_type, '.mat']])
        processed_data = load([iucn_data_params.processed_iucn_data_filename_prefix, system_type, '.mat']);
    ...
    processed_data.iucn_data = build_iucn_data(iucn_data_params, un_to_iucn_codes, industry_characteristics, global_params.system_type);
    ...
    processed_data.industry_data = build_industry_data(processed_data.iucn_data, ...
                                                       ...
                                                       iucn_data_params, un_to_iucn_codes, ...
    ...
    if (iucn_data_params.overwrite_tensors == true)
        ...
        build_iucn_tensors(processed_data, iucn_data_params, global_params.system_type);
    end
    ...
    if (iucn_data_params.save_processed_data == true)
        ...
        save([iucn_data_params.processed_iucn_data_filename_prefix, '_', global_params.system_type, '.mat'], 'processed_data', '-v7.3');
    end
...
function processed_industry_data = build_industry_data(processed_iucn_data, threat_concordance, iucn_data_params, un_to_iucn_codes, industry_characteristics, system_type)
    ...
    processed_industry_data.x = reorder_to_iucn(industry_characteristics.x_un, processed_iucn_data.country_characteristics.iucn_country_code_names, iucn_codes);
    
    if strcmp(system_type, 'eora') 
        processed_industry_data.x_names = reorder_to_iucn(industry_characteristics.x_un_names, processed_iucn_data.country_characteristics.iucn_country_code_names, iucn_codes);
    ...
    if (iucn_data_params.include_ghg == true)
        ...
        if strcmp(system_type, 'eora') 
                ...
                processed_industry_data.ghg = reorder_to_iucn(industry_characteristics.ghg, processed_iucn_data.country_characteristics.iucn_country_code_names, iucn_codes);
                processed_industry_data.global_ghg = sum(cell2mat(processed_industry_data.ghg)); 
...
function iucn_data = build_iucn_data(iucn_data_params, un_to_iucn_codes, industry_characteristics, system_type)
    
    disp(['processing iucn data to ', system_type, ' specification...'])
        
    iucn_data = struct();
        
   	if strcmp(iucn_data_params.iucn_data_type, 'new')  
        
        iucn_data.iucn_threat_data = read_raw_iucn_threat_data(iucn_data_params.iucn_data_threats_filename);
    	iucn_data.iucn_species_data = process_raw_iucn_species_data(iucn_data_params.iucn_data_species_filename, iucn_data.iucn_threat_data, ...
                                                                              industry_characteristics.industry_codes_to_use, un_to_iucn_codes.iucn_country_codes);
    else
    	iucn_data = initialise_object_from_old_data(iucn_data_params.old_iucn_data_filename);
    end

    disp(['processed raw iucn data at ', num2str(toc)])
    
    iucn_data.country_characteristics = build_iucn_country_characteristics(iucn_data_params.read_iucn_countries_from_file, iucn_data_params.iucn_data_type, un_to_iucn_codes, ...
                                                                                     iucn_data.iucn_species_data.iucn_country_codes_list, iucn_data.iucn_species_data.country_names_list);
                                                                                 
    iucn_data.threat_characteristics = build_threat_categories(iucn_data_params.old_threat_cause_class_filename, iucn_data_params.greenhouse_flag_filename, ...
                                                                iucn_data_params.read_threat_classification_from_file, iucn_data.iucn_threat_data);
    
    iucn_data = build_iucn_species_indexes(iucn_data);
...
function threat_concordance = build_threat_concordance(processed_iucn_data, iucn_data_params, un_to_iucn_codes, system_type)
    ...
    if strcmp(system_type, 'hscpc')
        threat_concordance = build_hscpc_threat_concordance(iucn_data_params.hscpc_concordance_filename, processed_iucn_data, iucn_data_params.hscpc_sector_num, ...
                                                                                      processed_iucn_data.threat_cause_class, processed_iucn_data.threat_num);
    elseif strcmp(system_type, 'eora') 
        threat_concordance = build_eora_threat_concordance(iucn_data_params.eora_concordance_file_prefix, processed_iucn_data.country_characteristics.iucn_country_code_names, ...
                                                                                    un_to_iucn_codes, processed_iucn_data.country_characteristics.ncoun);
    end
end


function build_iucn_tensors(processed_data, iucn_data_params, system_type)
    ...
    if strcmp(iucn_data_params.tensor_scale, 'country')
        ...
        for country_index = 1:processed_data.iucn_data.country_characteristics.ncoun
            ...
            current_tensor_filename = [iucn_data_params.iucn_tensor_file_prefix, system_type, '_', processed_data.iucn_data.country_characteristics.iucn_country_code_names{country_index}, '.mat'];
...
function iucn_country_characteristics = build_iucn_country_characteristics(read_iucn_countries_from_file, iucn_data_type, un_to_iucn_codes, country_codes_list, country_names_list)
    ...
    if read_iucn_countries_from_file
       ...
       elseif strcmp(iucn_data_type, 'new')
        ...
        [iucn_country_code_names, unique_inds, ~] = unique(country_codes_list);
        iucn_country_names = country_names_list(unique_inds); 
        empty_names = cellfun('isempty', iucn_country_code_names);

    end
    
    iucn_country_characteristics = struct();
    iucn_country_characteristics.iucn_country_code_names = iucn_country_code_names(~empty_names);
    iucn_country_characteristics.iucn_country_names = iucn_country_names(~empty_names);
    iucn_country_characteristics.ncoun = length(iucn_country_characteristics.iucn_country_code_names);
...
function iucn_species_data = process_raw_iucn_species_data(new_iucn_data_species_filename, iucn_threat_data, industry_codes_to_use, iucn_country_codes)
    ...
    fid = fopen(new_iucn_data_species_filename);
        raw_iucn_species_data = textscan(fid,'%f %f %s %s %s %s %s %s %s %s %s %s %s %s %s', 'HeaderLines', 1, 'delimiter', ';');
    fclose(fid);
    
    iucn_species_data_species_taxons = raw_iucn_species_data{2};
    iucn_species_data_country_codes = raw_iucn_species_data{12};
    iucn_species_data_country_names = raw_iucn_species_data{13};
    
    [iucn_species_data_country_codes, sorted_iucn_code_inds] = sort(iucn_species_data_country_codes);
    iucn_species_data_country_names = iucn_species_data_country_names(sorted_iucn_code_inds);
    iucn_species_data_species_taxons = iucn_species_data_species_taxons(sorted_iucn_code_inds);
    ...
    iucn_inds_to_use = ismember(iucn_species_data_country_codes, iucn_codes_to_use); %find all intersecting countries in iucn_list and UN list, i.e. remove iucn countries that there is no industrial data for.

    iucn_species_data_country_codes = iucn_species_data_country_codes(iucn_inds_to_use);
    iucn_species_data_species_taxons = iucn_species_data_species_taxons(iucn_inds_to_use);
    row_length = length(iucn_species_data_country_codes);

    display('processing raw iucn data')
    [expanded_iucn_threats, iucn_lengths] = build_iucn_threat_data(iucn_threat_data.iucn_threats, iucn_threat_data.iucn_threat_taxons, iucn_species_data_species_taxons, row_length);    
    [expanded_iucn_status, ~] = build_iucn_threat_data(iucn_threat_data.iucn_threat_status, iucn_threat_data.iucn_threat_taxons, iucn_species_data_species_taxons, row_length);
    ...
    iucn_species_data.excluded_countries = unique(iucn_species_data_country_names(~iucn_inds_to_use));
    ...
    iucn_species_data.country_names_list = expand_iucn_species_data(iucn_species_data_country_names(iucn_inds_to_use), row_length, iucn_lengths);
    iucn_species_data.iucn_taxons_list = expand_iucn_species_data(iucn_species_data_species_taxons, row_length, iucn_lengths);
    iucn_species_data.iucn_country_codes_list = expand_iucn_species_data(iucn_species_data_country_codes, row_length, iucn_lengths);
...
function processed_iucn_data = build_iucn_species_indexes(processed_iucn_data)
    ...
    processed_iucn_data.iucn_species_data.country_indexes_list = build_country_indexes_list(processed_iucn_data.iucn_species_data.iucn_data_length, processed_iucn_data.country_characteristics.ncoun, ...
                                                                            processed_iucn_data.country_characteristics.iucn_country_code_names, processed_iucn_data.iucn_species_data.iucn_country_codes_list);
