function MRIO_outputs = analyse_MRIO_output_routines(analyse_MRIO_params, IUCN_data_object)
    ...
    if analyse_MRIO_params.write_consumption_country_ranks == true
        ...
        ranked_consumption_country_data = write_ranked_outputs('by_country', 'consumption', analyse_MRIO_params, industry_characteristics, species_characteristics, ranked_consumption_countries, ... 
    ...
    if analyse_MRIO_params.write_production_country_ranks == true
        ...
        ranked_production_country_data = write_ranked_outputs('by_country', 'production', analyse_MRIO_params, industry_characteristics, species_characteristics, ranked_production_countries, ... 
    ...
    if analyse_MRIO_params.write_net_country_ranks == true
        ...
        ranked_consumption_country_data = append_zero_block(ranked_consumption_country_data, industry_characteristics, ranked_consumption_countries);
        ranked_production_country_data = append_zero_block(ranked_production_country_data, industry_characteristics, ranked_production_countries);
        
        [~, ~, sorted_production_inds] = intersect(ranked_consumption_country_data(:, 1), ranked_production_country_data(:, 1), 'stable');
        
        ranked_production_country_data = ranked_production_country_data(sorted_production_inds, :);
        net_country_data = cell2mat(ranked_consumption_country_data(:, 2:end)) - cell2mat(ranked_production_country_data(:, 2:end));
        
        [~, sorted_country_indexes] = sort(net_country_data(:, 1), 'ascend');
        ...
        table_names = {'Country', 'Net_Threats', 'Unique_Threatened_Species', 'Threatened_Animalia',  'Threatened_Plantae', 'Threatened_Fungi', 'Threatened_Chromista'};
    ...
    if analyse_MRIO_params.write_industry_ranks == true
        ...
        ranked_industry_data = write_ranked_outputs('by_industry', '', analyse_MRIO_params, industry_characteristics, species_characteristics, ranked_aggregated_industries_per_path, ranked_aggregated_threat_proportions_per_industry, industry_grouped_MRIO_species_aggregates);
...
function write_species_level_ranked_outputs(industry_characteristics, species_characteristics, analyse_MRIO_params, ranked_industry_paths_per_species, ranked_aggregated_threats_per_species, grouped_MRIO_species_aggregates)
    ...
    ranked_MRIO_industries = cell(length(ranked_industry_paths_per_species), 5);
    ...
    ranked_MRIO_industries(:, 5) = num2cell(round(ranked_aggregated_threats_per_species, 1));
    
    decomposed_aggregate_species_characteristics = cell(length(grouped_MRIO_species_aggregates), 5);
    ...
    ranked_MRIO_industries = [ranked_MRIO_industries decomposed_aggregate_species_characteristics];
     ...
     table_names = {'Consumption_Country', 'Consumption_Industry', 'Production_Country', 'Production_Industry','Total_Threats', 'Unique_Threatened_Species', 'Threatened_Animalia',  'Threatened_Plantae', 'Threatened_Fungi', 'Threatened_Chromista'};
...
function ranked_countries = append_zero_block(ranked_countries, industry_characteristics, country_indexes_used)
    ...
    cellblock_to_append = cell(length(consumption_countries_to_append), 7);
    cellblock_to_append(:, 1) = industry_characteristics.country_index_map(consumption_countries_to_append, 1);
        
    cellblock_to_append(:, 2:7) = num2cell(zeros(size(cellblock_to_append, 1), (size(cellblock_to_append, 2) - 1)));
...
function decomposed_aggregate_species_characteristics = decompose_aggregated_species(industry_grouped_species_aggregates, species_characteristics)
    ...
    decomposed_aggregate_species_characteristics = cell(length(industry_grouped_species_aggregates), 5);
    decomposed_aggregate_species_characteristics(:, 1) = num2cell(cellfun(@(x) length(unique(cell2mat(x))), industry_grouped_species_aggregates));
    decomposed_aggregate_species_characteristics(:, 2) = num2cell(cellfun(@(x) length(unique(cell2mat(cellfun(@(y) y(strcmp(species_characteristics.species_kingdom(y), 'ANIMALIA')), x, 'un', 0) ))), industry_grouped_species_aggregates));
    decomposed_aggregate_species_characteristics(:, 3) = num2cell(cellfun(@(x) length(unique(cell2mat(cellfun(@(y) y(strcmp(species_characteristics.species_kingdom(y), 'PLANTAE')), x, 'un', 0) ))), industry_grouped_species_aggregates));
    decomposed_aggregate_species_characteristics(:, 4) = num2cell(cellfun(@(x) length(unique(cell2mat(cellfun(@(y) y(strcmp(species_characteristics.species_kingdom(y), 'FUNGI')), x, 'un', 0) ))), industry_grouped_species_aggregates));
    decomposed_aggregate_species_characteristics(:, 5) = num2cell(cellfun(@(x) length(unique(cell2mat(cellfun(@(y) y(strcmp(species_characteristics.species_kingdom(y), 'CHROMISTA')), x, 'un', 0) ))), industry_grouped_species_aggregates));
    
end

function ranked_MRIO_industries = write_ranked_outputs(rank_type, file_identifier, analyse_MRIO_params, industry_characteristics, species_characteristics, object_to_aggregate_over, net_threat_aggregate, industry_grouped_species_aggregates)
    
    if strcmp(rank_type, 'by_industry')
        ranked_MRIO_industries = cell(length(object_to_aggregate_over), 3);
        ...
        ranked_MRIO_industries(:, 3) = num2cell(round(net_threat_aggregate, 1));
        table_names = {'Consumption_Country', 'Consumption_Industry', 'Total_Threats', 'Unique_Threatened_Species', 'Threatened_Animalia',  'Threatened_Plantae', 'Threatened_Fungi', 'Threatened_Chromista'};
        ...
        ranked_MRIO_industries(:, 2) = num2cell(round(net_threat_aggregate, 1));
        table_names = {'Consumption_Country', 'Total_Threats', 'Unique_Threatened_Species', 'Threatened_Animalia',  'Threatened_Plantae', 'Threatened_Fungi', 'Threatened_Chromista'};
...
function expanded_MRIO_data = write_expanded_ranked_outputs(ranked_MRIO_industries, analyse_MRIO_params, industry_characteristics, species_characteristics, grouped_MRIO_industry_paths, industry_grouped_MRIO_species_aggregates, grouped_MRIO_industry_vals)
        ...
        expanded_MRIO_data{3} = cellfun(@(x) num2cell(round(x, 1)), grouped_MRIO_industry_vals, 'un', 0); 
        expanded_MRIO_data{4} = cellfun(@(x) cellfun(@(y) length(y), x,'un',0), industry_grouped_MRIO_species_aggregates,'un', 0);
    
        expanded_MRIO_data{5} = cellfun(@(x) cellfun(@(y) length(find(strcmp(species_characteristics.species_kingdom(y), 'ANIMALIA'))), x,'un', 0), industry_grouped_MRIO_species_aggregates,'un', 0);
        expanded_MRIO_data{6} = cellfun(@(x) cellfun(@(y) length(find(strcmp(species_characteristics.species_kingdom(y), 'PLANTAE'))), x,'un', 0), industry_grouped_MRIO_species_aggregates,'un', 0);
        ...
        T = cell2table(expanded_MRIO_data, 'VariableNames', {'Consumption_Country', 'Consumption_Industry', 'Total_Threats', 'Total_Threatened_Species', 'Total_Threatened_Animalia', ...
                                                            'Total_Threatened_Plantae', 'Total_Threatened_Fungi', 'TTotal_hreatened_Chromista', 'Production_Country', 'Production_Industry', ...
